...
class _DLTrainAdapter:
    def __init__(self, inner: Any, epochs: int, batch_size: int):
        self.inner = inner
        self.epochs = epochs
        self.batch_size = batch_size

    def fit(self, df: pd.DataFrame, y: pd.Series) -> None:
        self.inner.train(df, y, epochs=self.epochs, batch_size=self.batch_size)
    ...
    *,
    ...
    for key in config.model_keys:
        if key == "lgb":
            ...
            m.feature_cols = list(cols)
            models[key] = _DLTrainAdapter(m, epochs=dl_epochs, batch_size=dl_batch_size)
        elif key == "cnn":
