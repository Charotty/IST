"""
Ensemble Trading Environment for RL-based risk management.

This environment uses ensemble probability instead of direction probability.
Designed for use after simple/regime ensemble implementation.
"""
...
class EnsembleTradingEnv:
    """
    RL environment using ensemble probability instead of direction probability.
    ...
    - vol_spike_prob, ensemble_prob (replaces direction_prob), meta_prob
    - ema_slope, adx, rsi_15m, rsi_4h
    """
    
    def __init__(self, df: pd.DataFrame, initial_balance: float = 10000.0):
        """
        Initialize the ensemble trading environment.
        
        Args:
            df: DataFrame with OHLCV data and features including:
                - final_signal: Signal from meta-learning (-1 to 1)
                - ensemble_prob: Ensemble probability (replaces direction_prob)
                - rsi, volatility, regime_pred
        ...
        self.required_columns = [
            ...
            'ema_slope', 'adx', 'rsi_15m', 'rsi_4h', 'close'
        ]
    ...
    def _get_state(self, step: int) -> np.ndarray:
        ...
        Returns:
            10-dimensional state vector with ensemble_prob
        """
        ...
        state = np.array([
            ...
            row['regime_pred'],
            row['vol_spike_prob'],
            row['ensemble_prob'],  # Replaces direction_prob
            row['meta_prob'],
            row['ema_slope'],
            row['adx'],
            row['rsi_15m'],
