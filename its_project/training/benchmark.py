...
class GTX1660Benchmark:
    ...
    def __init__(self, output_dir: str = "benchmark_results"):
        ...
        self.memory_monitor = MemoryMonitor(max_memory_gb=4.0)  # 4GB for GTX 1660 Ti
        ...
        class BERTModel(nn.Module):
            def __init__(self):
                ...
                self.transformer = nn.TransformerEncoder(
                    ...
                    ), num_layers=2  # Reduced from 12 to 2 for testing
                )
    ...
    def _create_small_llm(self) -> nn.Module:
        ...
        class SmallLLM(nn.Module):
            def __init__(self):
                ...
                self.lstm = nn.LSTM(512, 512, num_layers=2, batch_first=True)  # Reduced from 6 to 2
                self.fc = nn.Linear(512, 1000)
            
            def forward(self, x):
                x = self.embedding(x)  # (batch, seq_len, embedding_dim)
                x, (h_n, c_n) = self.lstm(x)  # x: (batch, seq_len, hidden_dim)
    ...
    def _benchmark_model(self, model: nn.Module, model_name: str, 
        ...
        # Create fresh model for each benchmark to avoid state issues
        if model_name == "resnet50":
            model_copy = self._create_resnet50()
        elif model_name == "bert-base":
            model_copy = self._create_bert_base()
        elif model_name == "small_llm":
            model_copy = self._create_small_llm()
        else:
            model_copy = model
        ...
        if 'resnet' in model_name:
            ...
            target = torch.randint(0, 1000, (config.max_batch_size,))
        else:  # small_llm
            sample_input = torch.randint(0, 1000, (config.max_batch_size, 128))
            target = torch.randint(0, 1000, (config.max_batch_size,))
        
        # Move to GPU and fix data types
        if torch.cuda.is_available():
            if 'resnet' in model_name:
                sample_input = sample_input.cuda().float()
            else:  # bert and small_llm use embedding layers
                sample_input = sample_input.cuda().long()
